...
impl<T> Queue<T> {
    pub fn push(&self, item: T) -> Result<(), &'static str> {
        let tail = unsafe {
            atomic_u64_unsync_load(&self.0.tail)
        };
    ...
    pub fn pop(&self) -> Option<T> {
        let mut head = self.0.head.load(Ordering::Acquire);
...
pub unsafe fn atomic_u64_unsync_load(atomic: &AtomicU64) -> u64 {
    *(atomic as *const AtomicU64).cast()
}

#[cfg(test)]
mod tests {
    use crate::u64_opt_steal::{pack, unpack, Inner, Queue, Steal, QUEUE_CAPACITY};
    ...
    fn with_steal() {
        ...
        println!("u64 opt per loop: {:}ms", duration);
