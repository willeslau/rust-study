...
struct Queue<T>(Arc<Mutex<Inner<T>>>);

impl<T> !Send for Queue<T> {}

struct Steal<T>(Arc<Mutex<Inner<T>>>);
...
#[cfg(test)]
mod tests {
    use crate::mu_steal::{Inner, Queue, Steal, QUEUE_CAPACITY};
    ...
    fn test_queue() {
        let inner = Inner::new();
        let queue = Queue::<TestItem>(Arc::new(Mutex::new(inner)));
    ...
    fn with_steal() {
        ...
        println!("mutex per loop: {:}ms", duration);
